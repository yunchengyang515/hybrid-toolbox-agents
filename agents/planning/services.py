...
class PlanningService:
    ...
    async def generate_plan(self, request: GeneratePlanRequest) -> GeneratePlanResponse:
        ...
        messages = self._build_plan_generation_messages(request)
        ...
        try:
            ...
            logger.error(f"Error parsing generated plan: {str(e)} \n Result: {result}")
            # Create a minimal valid plan as fallback
            training_plan = TrainingPlan(
                title="Error in Plan Generation",
                description="Failed to generate a valid plan. Please try again.",
                weeks=[]
            )
        ...
        # Generate recommendations based on the profile and plan
        recommendations = await self._generate_plan_recommendations(request.profile, training_plan)
...
- Include detailed warmups: {request.plan_parameters.include_warmups}
- Include cooldowns: {request.plan_parameters.include_cooldowns}
...
"""
    ...
    async def _generate_plan_recommendations(self, profile: Dict[str, Any], plan: TrainingPlan) -> List[str]:
        """Generate additional recommendations based on the profile and generated plan."""
        # Create a prompt for recommendations
        prompt = f"""Based on the user's profile and the training plan, provide 3-5 additional recommendations.

User Profile Summary:
- Training history: {profile.get('training_history', 'Not specified')}
- Fitness background: {profile.get('fitness_background', 'Not specified')} 
- Goals: {profile.get('training_goals', 'Not specified')}
- Health constraints: {profile.get('health_constraints', 'Not specified')}

Training Plan: {plan.title} - {plan.description}

Return ONLY the numbered list of recommendations.
"""
        messages = [
            Message(role=Role.SYSTEM, content=self.config.recommendations_system_prompt),
            Message(role=Role.USER, content=prompt)
        ]
